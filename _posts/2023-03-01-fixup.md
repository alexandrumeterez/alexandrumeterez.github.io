...
$$
\begin{align}
	\left\Vert \frac{\partial l}{\partial x_{i-1}} \right\Vert \geq \frac{l(z, y) - H(p)}{\| x_{i-1} \|}
\end{align}
$$
...
Let $\left\Vert \frac{\partial l(x_{i-1}, y)}{\partial x_{i-1}} \right\Vert$ be the norm of the loss gradient wrt $x_{i-1}$ and $\left\Vert \frac{\partial l(x_{i-1} + t v, y)}{\partial t} \right\Vert$ at $t = 0$, be the norm of the directional derivative of the loss in the direction $v = \frac{x_{i-1}}{\| x_{i-1} \|}$ (definition from [https://en.wikipedia.org/wiki/Directional_derivative#Derivatives_of_scalar_valued_functions_of_vectors]). 

Therefore, we obtain:

$$
\begin{align}
\left\Vert \frac{\partial l(x_{i-1}, y)}{\partial x_{i-1}} \right\Vert &\geq \left\Vert \frac{\partial l(x_{i-1} + t v, y)}{\partial t} \right\Vert \\
&= \left\Vert \frac{\partial l(x_{i-1} + t \frac{x_{i-1}}{\| x_{i-1} \|}, y)}{\partial t} \right\Vert \\
&= \left\Vert \frac{\partial l(x_{i-1} + \epsilon x_{i-1}, y)}{\partial t} \right\Vert \frac{\partial \epsilon}{\partial t} & \epsilon = \frac{t}{\| x_{i-1} \|} \\
...
This theorem bounds $\left\Vert \frac{\partial l_{avg}}{\partial \theta_{ph}} \right\Vert$, so the gradient of the loss wrt p.h. sets in the network. Proof is similar to before (see paper).
